...
describe("Integration: Program Loading Flows", () => {
    ...
    test("loads program from URL parameter and saves to cache", async () => {
        window.location = new URL("https://example.com/?url=https://docs.google.com/spreadsheets/d/test-url");
        ...
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("test-url"), expect.anything());
    ...
    test("loads program from localStorage if no URL parameter", async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("cached-url"), expect.anything());
    ...
    test("falls back to programCache on fetch failure", async () => {
        ...
        localStorage.setItem("programCache", JSON.stringify([
            { key: "unitName", value: "Offline Ward" }
        ]));
